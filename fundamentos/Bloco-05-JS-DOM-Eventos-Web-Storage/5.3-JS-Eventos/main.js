...
// A primeira caixa está um pouco acima das outras pois a classe dela tem uma transformação

// 2. Crie uma função que adicione a classe 'tech' ao elemento `li` quando for clicado.
// 2.1. Deve existir apenas um elemento com a classe 'tech'. Como você faz isso?

let container = document.getElementsByClassName('container')[0].children;

for (let index = 0; index < container.length; index += 1) {
    container[index].addEventListener('click',mudaClass);
}
function mudaClass (opcao) {
    let target = opcao.target;
    if (target.className == 'tech') {
        return
    } else {
        let container = document.getElementsByClassName('container')[0].children;
        for (let index in container) {
            if (container[index].id != target.id) {
                container[index].className = '';
            }
        }
...
myWebpage.addEventListener('dblclick', meuSite);
